    ...
    await tb_evento.findOne(filtro, {}, function(err, result) {
        ...
        if (result) {
            ...
            var options = {
              ...
              'headers': {
                'Content-Type': 'application/json',
                's_token': result.token_sympla //TokenSympla
              }
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
...
//PegaChat
app.get('/PegaChat/:IdEvento', async (req, res, next) => {

  var tb_chat = conn.collection('tb_chat');
  var IdEvento = req.params.IdEvento;

  await tb_chat.find({id_tb_evento: Mongoose.Types.ObjectId(IdEvento), c_visivel: 1}, {sort: {c_data: 1}, limit: 50}).toArray(function(err, result) {
    if (err) throw err;
    if (result) {
  ...
  //TestPHP
  app.post('/TestPHP', async (req, res, next) => {

    console.log("TestPHP" + req.method);
    return res.send("TestPHP" + req.method);
    
  });

  app.get('/phpcallback', function(req, res) {
...
io.sockets.on('connection', function (client) {
    ...
    */
    ...
    app.post('/Login/:IdEvento', async (req, res, next) => {
      ...
      var c_token_sympla = req.body.c_token_sympla;
      ...
      await tb_evento.findOne({id_sympla: parseInt(IdEvento)}, {}, function(err, resultEvento) {
        if (err) throw err;
          if (resultEvento) {
            console.log('tb_evento', resultEvento._id);
            console.log('tb_evento', resultEvento.c_nome);

            const testexxx = resultEvento._id;
                          ...
                          if(val.id == undefined){
                            ...
                            if(IdIngresso === DadosIngresso.ticket_number){
                              ...
                              tb_acesso.insertOne({
                                ...
                                c_evento: IdEvento,
                                c_nome_evento: resultEvento.c_nome,
                                c_id_evento: resultEvento._id,
  ...
  //AtualizarPagina
  app.post('/AtualizarPagina/:IdEvento', async (req, res, next) => {
    
    var IdEvento = req.params.IdEvento;
    let ListaConfig = JSON.stringify({
      c_evento: IdEvento
    });
    io.emit("AtualizarPagina", ListaConfig);

    console.log("AtualizarPagina - " + IdEvento);
    return res.send("AtualizarPagina - " + IdEvento);
